...
if (!empty($_GET['year']) || !empty($_GET['month']) || !empty($_GET['day'])){
    ...
    echo "<h1>PERPETUAL <hr class='new1'></h1>";
...
<form action="./perpetual.php" method="GET">

<?php
if (($month-1)>0) {
?>

    <a href="perpetual.php?day=<?=$day;?>&month=<?=($month-1);?>&year=<?=$year;?>">
<?php
    echo "<input type='button' class='button' name='' value='◀ 上一月 '>";
    echo "</a>"; 
    }else {
?> 
    <a href="perpetual.php?day=<?=$day;?>&month=<?=12;?>&year=<?=($year-1);?>">
<?php
    echo "<input type='button' class='button' name='' value='◀ 上一月 '>";
    echo "</a>";
    }
...
<a href="perpetual.php?day=<?=date('d');?>&month=<?=date('m');?>&year=<?=date('Y');?>">
...
?>
        ...
        <a href="perpetual.php?day=<?=$day;?>&month=1&year=<?=($year+1);?>">
